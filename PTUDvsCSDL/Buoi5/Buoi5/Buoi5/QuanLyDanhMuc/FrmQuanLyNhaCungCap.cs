...
namespace Buoi5.QuanLyDanhMuc
{
    public partial class FrmQuanLyNhaCungCap : Form
    {
        ...
        {
            if (checkForm() == false) return;
            Database.Execute("update NhaCungCap set TenNhaCungCap = N'"+txtTen.Text+"', DiaChi = N'"+txtDiaChi.Text+"', DienThoai = "+mtxtSDT.Text+" where MaNhaCungCap = "+dgvNhaCungCap.CurrentRow.Cells["colMaNhaCungCap"].Value.ToString()+"");
            loaddgvNhaCungCap();
        }
